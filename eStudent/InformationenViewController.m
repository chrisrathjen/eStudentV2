...
//Lädt sämtliche POI-Daten. Alle Views werden programmatisch erzeugt.
- (void)viewDidLoad
{
    [super viewDidLoad];
...
//
    ...
    UIView *poiView = [[UIView alloc] init]; //Der View, der die Details des POI beinhalten soll.
    [poiView sizeToFit];
    ...
    //Fügt evtl. eine Grafik vor den Titel.
    ...
    //Bei dem POI handelt es sich um ein Gebäude. Die Haltestellen werden absteigend nach der Entfernung zum POI sortiert.
    if (_haltestellen && [pointOfInterest.type intValue] == 0)
    {
        ...
        //Nur die drei nächstliegenden Haltestellen sollen aufgelistet werden, weil sonst die Liste zu lang sein würde.
        for (int i = 0; i < 3; i++)
        {
...
//Wischt der Benutzer nach Links, wird direkt die Kartenansicht geladen.
...
//Wischt der Benutzer nach rechts, gelangt er zurück zum vorigen Controller.
...
//Öffnet die Webseite des POI.
- (void)openWebsite:(id)sender
{
    [[UIApplication sharedApplication] openURL:[NSURL URLWithString:pointOfInterest.web]];
}

//Startet einen Telefonanruf an die Telefonnummer des POI.
- (void)callPhoneNumber:(id)sender
{
...
//Die Telefonnummer des POI wird nur angerufen, wenn der Nutzer dies erneut bestätigt.
...
//Hebt eine angewählte Zelle visuell hervor. Da die Zellen manuell erstellt wurden, muss dies hier manuell gemacht werden.
- (void)highlightCell:(UILongPressGestureRecognizer *)sender
{
...
//Hebt eine angewählte Haltestellen-Zelle visuell hervor. Da die Zellen manuell erstellt wurden, muss dies hier manuell gemacht werden.
- (void)highlightBusStopCell:(UILongPressGestureRecognizer *)sender
{
    if (sender.state == UIGestureRecognizerStateBegan)
    {
...
//Eine Einrichtung hat ein Gebäude. Der Nutzer tippt auf die entsprechende Zelle um sich die Informationen des Gebäudes
//anzeigen zu lassen. Der korrespondierende POI wird in einem weiteren InformationViewController geladen.
- (void)showBuilding:(UITapGestureRecognizer *)sender
{
    sender.view.layer.opacity = .5;
    InformationenViewController *ivc = [[InformationenViewController alloc] initWithNibName:@"Informationen" bundle:nil];
...
//Bei dem POI handelt es sich um ein Gebäude mit Haltestellen. Der Nutzer tippt auf eine der Haltestellen-Zellen, um sich
//den korrespondierenden POI anzeigen zu lassen. Ein weiterer InformationViewController wird geladen.
