...
//Das Erstellen/Bearbeiten des Eintrags wird vom Nutzer abgebrochen.
- (IBAction)cancelButtonPressed:(id)sender
{
...
//Der Eintrag und sämtliche dazugehörige Daten, wie z.B. die Kriterien werden gespeichert.
- (IBAction)fertigButtonPressed:(id)sender
{
...
//Speichert den Eintrag in die Datenbank über den Datenmanager. Die Kriterien werden ebenfalls gespeichert, falls welche angelegt wurden.
- (void)saveEintragToDatabaseSaveAllKriterien:(BOOL)saveAll
{
    if (eintrag)
    {
        if (eintrag.lecture) //Die dazugehörige Veranstaltung wird ebenfalls mit angepasst.
        {
            eintrag.lecture.title = eintragsTitel;
            eintrag.lecture.cp = [NSNumber numberWithInt:[eintragsCP intValue]];
            eintrag.lecture.type = eintragsArtString;
        }
...
- (UITableViewCell *)tableView:(UITableView *)_tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        ...
        else if (indexPath.row == 5)
        {
            ...
            benotetSwitch = [[UISwitch alloc] initWithFrame:CGRectZero];
            ...
            cell.accessoryView = benotetSwitch;
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            cell.accessoryType = UITableViewCellAccessoryNone;
        }
        else if (indexPath.row == 6)
        {
            ...
            cell.textLabel.font = [UIFont boldSystemFontOfSize:11.0];
            bestandenSwitch = [[UISwitch alloc] initWithFrame:CGRectZero];
            ...
            cell.accessoryView = bestandenSwitch;
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            cell.accessoryType = UITableViewCellAccessoryNone;
        }
...
//Kriterien können per Wischgeste gelöscht werden.
...
//Wird ausgeführt, wenn ein Kriterium gelöscht wurde - das Kriterium muss gelöscht werden.
...
//Das übergebene Datum wird normalisiert.
- (NSDate *)normalizedDateWithDate:(NSDate *)date
{
...
//Reagiert darauf, wenn eine Zelle ausgewählt wurde. Je nachdem um welche Zelle es sich handelt, passieren unterschiedliche Dinge.
- (void)tableView:(UITableView *)_tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0)
    {
        ...
        else if (indexPath.row == 1) //Eintrags-Art soll ausgewählt werden
        {
        ...
        else if (indexPath.row == 3) //Studiengang soll ausgewählt werden
        {
        ...
        else if (indexPath.row == 4) //Semester soll ausgewählt werden
        {
...
//Der Switch, ob der Eintrag benotet ist.
- (void)benotetSwitched:(UISwitch *)sender
{
    if (sender.on)
    {
...
//Der Switch, ob der Eintrag bereits bestanden ist.
- (void)bestandenSwitched:(UISwitch *)sender
{   
    if (sender.on)
    {
...
#pragma mark - Delete Eintrag Button States
...
//Der Nutzer tippt auf den 'Eintrag löschen' Button, dies muss erst vom Nutzer bestätigt werden.
...
//Reagiert darauf, ob der Nutzer den Löschvorgang des Eintrags bestätigt.
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 0) //Eintrag löschen bestätigt
    {
        if (eintrag)
        {
            ...
            for (int i = 0; i < _semesters.count; i++)
            {
                ...
                if ([s isEqual:currentSemester])
                {
                    [[CoreDataDataManager sharedInstance] deleteAllEmptyFutureSemestersAfterIndex:i]; //Der Datenmanager löscht den Eintrag aus der Datenbank.
                    break;
                }
            }
        }
...
//Der Zustand des 'Fertig' Button ist von verschiedenen Parametern abhängig. Diese Parameter werden hier geprüft und entsprechen wird
//der Button aktiviert oder deaktiviert.
...
//Wird aufgerufen, wenn die Tastatur hochgefahren wird.
- (void)keyboardWasShown:(NSNotification*)notification
{
...
//Wird aufgerufen, wenn die Tastatur runtergefahren wird.
- (void)keyboardWillBeHidden:(NSNotification*)notification
{
