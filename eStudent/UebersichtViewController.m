...
//Pro Studiengang wird ein Scrollview mit den Einträgen dieses Studiengangs gefüllt. Ein Scrollview wird pro Page angezeigt.
- (void)loadView:(UIScrollView *)scrollView Studiengang:(Studiengang *)studiengang
{
    _semester = nil;
    ...
    double yOffset = 10.0;
    if (_semester.count > 0) //Jedes Semester wird mit einem Label von den anderen abgegrenzt.
    {
...
//Der '+' Button um einen Eintrag manuell anzulegen wurde betätigt. Der entsprechende Controller wird geladen und dem Nutzer präsentiert.
- (void)addEntry:(id)sender
{
...
//Der `Auge` Button zum Filtern der Einträge wurde betätigt. Der entsprechende Controller wird geladen und dem Nuter präsentiert.
- (void)filterEntries:(id)sender
{
...
//Fährt unter dem Eintragsview die Bearbeitunsleiste aus.
- (void)showEditEntryView:(UITapGestureRecognizer *)sender
{
...
//Ein Studiengang wurde per wischen zur Seite gewechselt.
- (void)scrollViewDidScroll:(UIScrollView *)sender
{
    ...
    if (tmp > _pageControl.currentPage) //wenn ein Studiengang nach links gewischt wird
    {
        Studiengang *s = (Studiengang *)[_studiengaenge objectAtIndex:_currentPage];
        _studiengangLabel.text = [NSString stringWithFormat:@"%@, %@", s.name, s.abschluss];
    }
    else if (tmp < _pageControl.currentPage) //wenn ein Studiengang nach rechts gewischt wird
    {
        Studiengang *s = (Studiengang *)[_studiengaenge objectAtIndex:_currentPage];
        _studiengangLabel.text = [NSString stringWithFormat:@"%@, %@", s.name, s.abschluss];
    }
