...
//Bereitet den Empty State vor, über den der Nutzer informiert wird, dass heute keine Veranstaltungen stattfinden.
- (id)initWithFrame:(CGRect)frame DateArray:(NSArray *)dates
{
    ...
    if (self)
    {
        ...
        textView.text = NSLocalizedString(@"Keine Veranstaltungen heute? Wische, um den Tag zu wechseln oder füge Veranstaltungen hinzu.", @"Keine Veranstaltungen heute? Wische, um den Tag zu wechseln oder füge Veranstaltungen hinzu.");
        
        CGRect _frame = noDatesToday.frame;
        _frame.origin.x = (_noDatesTodayView.frame.size.width / 2.0) - (_frame.size.width / 2.0);
        _frame.origin.y = (_noDatesTodayView.frame.size.height / 2.0) - (_frame.size.height/1.5) - 30.0;
        noDatesToday.frame = _frame;
        [_noDatesTodayView addSubview:noDatesToday];
        
        textView.frame = CGRectMake(20.0, _frame.origin.y + _frame.size.height + 10.0, 280.0, 100.0);
...
//Überschreibt den Setter: das UI wird neu geladen.
...
//Erstellt die Termine für das UI.
...
//Lädt die Daten neu, wenn der Nutzer eine Woche zurückgeht.
...
//Lädt die Daten neu, wenn der Nutzer eine Woche vorgeht.
...
//Reagiert darauf, wenn der Text einer Notiz verändert wird.
...
//Wenn eine Notiz ausgefahren wird, wird der Cursor in das entsprechende Textfeld gesetzt.
