...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0) //Essen
    {
        ...
        if ([[NSUserDefaults standardUserDefaults] objectForKey:kDEFAULTS_MENSA_NAME])
        {
            ...
            {
                [cell.activitiyIndicator stopAnimating];
                /*UIImageView *mensaClosedImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"mensa-closed"]];
                 CGRect frame = mensaClosedImage.frame;
                 frame.origin.y = cell.essenFunktionLabel.frame.size.height + 20.0;
                 frame.origin.x = cell.frame.size.width/2.0 - frame.size.width/2.0;
                 mensaClosedImage.frame = frame;
                 [cell addSubview:mensaClosedImage];*/
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0) //Essen
    {
        ...
        {
#warning uncomment for iOS7only            
            //[cell.essensBeschreibungsTextView.layoutManager ensureLayoutForTextContainer:cell.essensBeschreibungsTextView.textContainer];
            //[cell.essensBeschreibungsTextView layoutIfNeeded];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (!settingsIsVisible)
    {
        ...
        {
            if (![[NSUserDefaults standardUserDefaults] objectForKey:kDEFAULTS_COURSE_OF_STUDY])
            {
